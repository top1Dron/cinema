    ...
    if (url != null){
        $.ajax({
            ...
            success: function(resp){
                $(button).parent().parent().html($('#main_gallery_empty_form').html().replace(/__prefix__/g, form_ind));
                var imageColumn = $('#main_gallery_images').children('.col-md-6').children('.col-md-6')[0];
                var img = $(imageColumn).children('.card').children('img')[0];
